      ...
      <div className="table-container border rounded-[1.04vw] ml-[1.5vw] mr-[1.5vw] mb-[1.5vw]  border-[#E0E0E9] overflow-hidden ">
        <table className="invoices-table w-[72.14vw] h-[1vw]" style={{ fontFamily: 'Switzer' }}>
                      ...
                      <div className="bg-white p-[2vw] rounded-[1vw] z-10  h-[12vw] w-[22vw]">
                        ...
                        <div className="mt-[0.5vw]">
                          <p
                            ...
                            style={{ fontFamily: "Switzer", fontWeight: 400, color: "#808080" }}
                          >
                            Are you sure you would like to delete this Invoice{" "}
                            <span className="flex justify-center"> from the database? This action cannot be{" "}</span>{" "}
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={7} className="py-[2vw] text-center text-[0.83vw] text-[#98A2B3]">
                  Loading invoices...
                </td>
              </tr>
            ) : invoices.length === 0 ? (
              ...
              invoices.map((invoice, index) => (
                <tr key={invoice.id} className={index === invoices.length - 1 ? "" : "border-b border-gray-200"}>
                  ...
                  <td className="flex items-center">
                    <Image src={pdf} alt="pdf icon" className="-ml-[6.5vw]" />
                    <div className="flex justify-center items-center">
                      <td className="font-medium text-center  text-[0.83vw] -ml-[0.6vw]">{`Zwilt Invoice # ${invoice.id.slice(-6)}`}</td>
                    </div>
                    {showConfirm && invoiceToDelete === invoice.id && (
                      <div className="fixed inset-0 flex items-center justify-center z-50 ">
                  ...
                  <td className="text-center  relative  ">
                    <div className="flex justify-center items-center">
                      <span className={`status px-[0.41vw] py-[0.20vw]  text-[0.83vw] border rounded-[2.60vw] font-medium  ${invoice.status?.toLowerCase() === 'paid' ? 'text-[#17B26A] border-[#abefc6] bg-[#dcfae6] font-normal px-[0.7vw]' : 'bg-gray-100 text-[#98A2B3] border-gray-300 font-normal px-[0.7vw]'}`}>
                        {invoice.status}
                      </span>
                    </div>
                  </td>
                    ...
                    <div className="flex relative items-center justify-center">
                      <div className="static group flex items-center justify-center w-[1.25vw] h-[1.25vw] overflow-hidden">
                        <button onClick={() => handleDelete(invoice.id)}>
                          <Image src={deleteicon} alt="Delete icon" height={24} width={24} className="relative z-10" />
                      ...
                      <div className="static group flex items-center justify-center ml-[0.8vw] w-[1.25vw] h-[1.25vw] overflow-hidden">
                        <button onClick={() => handleDownload(invoice)} className="flex   static items-center justify-center ">
                          <Image src={download} alt="Download icon" height={24} width={24} className="-mt-[1.69vw]" />
